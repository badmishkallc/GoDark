...
{
    /// <summary>
    /// A block cipher symmetric alogrithm created by Bruce Schneier.
    /// </summary>
    /// <remarks>
    ///     <para>
    ///     BlowFish is a block cipher created by Bruce Schneier. Mr. Schneier Keeps blowfish resources
    ///     handy on his website. https://www.schneier.com/cryptography/blowfish/
    ///     </para>
    ///     <para>
    ///     One thing to note is that this cipher is commonly implemented using BigEndian.  However,
    ///     there are JavaScript versions that use LittleEndian.  
    ///     </para>
    ///     <para>
    ///     Thus, if you want to <strong>get one that can do both</strong> and is built upon <see cref="SymmetricAlgorithm"/>
    ///     then use GoDark's <see cref="BlowFish"/> implementation. 
    ///     </para>
    /// </remarks>
...
#pragma warning restore  CS0109
        ...
        private class BlowFishTransform : ICryptoTransform
        {
            private static uint[] s_p =  {
            ...
            public bool IsEncryptor { get; private set; }
            ...
            private static void ThrowIfNotBlockSize(string name, int value)
            {
                if(!(value % 8 == 0))
                {
                    throw new ArgumentException($"{name} must be a multiple of the BlockSize(8)");
                }
            }
            ...
            private static void EncryptBlock(byte[] block, uint[] blockP, uint[,] sBox,  bool isLittleEndian)
            {
                var surrogatePair = ReadSurrogatePair(block, isLittleEndian);
                Encrypt(surrogatePair, blockP, sBox);
                WriteSurrogatePair(block, surrogatePair, isLittleEndian);
            }

            private static void DecryptBlock(byte[] block, uint[] blockP, uint[,] sBox,  bool isLittleEndian)
            {
                var surrogatePair = ReadSurrogatePair(block, isLittleEndian);
                Decrypt(surrogatePair, blockP, sBox);
                WriteSurrogatePair(block, surrogatePair, isLittleEndian);
            }

            private static uint[] ReadSurrogatePair(byte[] block, bool littleEndian)
            {
                uint[] surrogatePair = new uint[] { 0, 0 };
                byte[] lo = new byte[4];
                byte[] hi = new byte[4];

                Buffer.BlockCopy(block, 0, lo, 0, 4);
                Buffer.BlockCopy(block, 4, hi, 0, 4);

                if(littleEndian)
                {
                    surrogatePair[0] = hi.ToUInt32();
                    surrogatePair[1] = lo.ToUInt32();

                    return surrogatePair;
                }

                surrogatePair[0] = lo.ToUInt32FromBigEndian();
                surrogatePair[1] = hi.ToUInt32FromBigEndian();

                return surrogatePair;
            }

            private static void WriteSurrogatePair(byte[] int64Set, uint[] surrogatePair, bool littleEndian)
            {
                byte[] lo, hi;

                if(littleEndian)
                {
                    lo = surrogatePair[1].ToBytes();
                    hi = surrogatePair[0].ToBytes();
                }
                else
                {
                    lo = surrogatePair[0].ToBigEndianBytes();
                    hi = surrogatePair[1].ToBigEndianBytes();
                }

                Buffer.BlockCopy(lo, 0, int64Set, 0, 4);
                Buffer.BlockCopy(hi, 0, int64Set, 4, 4);
            }

            private void SetupKey(byte[] originalKey)
            {
                if (originalKey.Length > 56)
                {
                    throw new Exception("The keysize must be 56 or less");
                }

                var key = new byte[originalKey.Length];
                var p = new uint[s_p.Length];

                Buffer.BlockCopy(originalKey, 0, key, 0, originalKey.Length);
                Buffer.BlockCopy(s_p, 0, p, 0, s_p.Length);

                uint[,] sBox = GenerateSBox();

                int j = 0,
                    l = originalKey.Length;
                ...
                uint[] surrgogatePair = new uint[] { 0, 0 };
                for(var i = 0; i < 18; i += 2)
                {
                    Encrypt(surrgogatePair, p, sBox);
                    p[i] = surrgogatePair[0];
                    p[i + 1] = surrgogatePair[1];
                }
                ...
                for(j = 0; j < 4; j++)
                {
                    ...
                    {
                        Encrypt(surrgogatePair, p, sBox);
                        sBox[j, i] = surrgogatePair[0];
                        sBox[j, i + 1] = surrgogatePair[1];
                    }
                }
            ...
            private static void Encrypt(uint[] surrogatePair, uint[] p, uint[,] sBox)
            {
                uint l = surrogatePair[0],
                     r = surrogatePair[1];

                // unint to avoid casting.
            ...
            private static void Decrypt(uint[] surrogatePair, uint[] p, uint[,] sBox)
            {
                uint l = surrogatePair[0],
                     r = surrogatePair[1];

                // unint is avoid casting.
            ...
            // This is commonly known as the F function. Since its typically frowned
            // upon to name a function with only one letter, its FFunction in .NET.
            private static uint FFunction(uint x, uint[,] sBox)
            {
                byte[] bytes = x.ToBigEndianBytes();
                uint h = sBox[0, bytes[0]] + sBox[1, bytes[1]];
                return h ^ sBox[2, bytes[2]] + sBox[3, bytes[3]];
            }
            ...
            protected virtual void Dispose(bool disposing)
            {
                if (!disposedValue)
                {
                    if (disposing)
                    {
                        ...
                        Array.Clear(this.sBox, 0, this.sBox.Length);
                        this.IV = null;
                        this.p = null;
                        this.sBox = null;
                    }
